  ...
  h.onMessage([&lane,&ref_vel,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
          ...
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	auto sensor_fusion = j[1]["sensor_fusion"];
                ...
                // handle surrounding cars sensor fusion data
		if(prev_size > 0){
		    car_s = end_path_s;
		}
		...
		// 0.224 ~= 5 meters per second sq
		    ...
		    ref_vel += 0.224;
          	...
          	// TODO: define a path made up of (x,y) points that the car will visit sequentially every .02 seconds
                ...
                // create some evenly spaced waypoints
		// interpolate with a spline and generate more points that control speed
		vector<double> ptsx;
		vector<double> ptsy;
		...
		// calculate how to break up spline points so that we travel at our desired refererence velocity
		double target_x = 30.0; // selected time horizon
		...
		// fill up the rest of our path planner after filling it with previous points, here we will always output 50 points
		    ...
		    double N = (target_dist/(0.02*ref_vel/2.24)); // converting from miles per hour to meters per second
